...
from dotenv import load_dotenv  # ✅ PRIDĖTA dotenv BIBLIOTEKA

# ✅ Teisingas kelias į `.env` failą
load_dotenv("C:/Users/37065/Projects/ai.assist/config/.env")

# ✅ Konfigūracijos failų keliai
CONFIG_FILES = ["config/ai_config.json", "config.json"]

def load_api_key():
    """Ieško OpenAI API rakto aplinkos kintamuosiuose (pirmenybė) ir konfigūracijos failuose."""
    api_key = os.getenv("OPENAI_API_KEY")  # ✅ PIRMA TIKRINAME `.env` FAILĄ
    ...
    if not api_key:
        raise ValueError("❌ API raktas nerastas! Patikrink .env arba konfigūracijos failus.")
